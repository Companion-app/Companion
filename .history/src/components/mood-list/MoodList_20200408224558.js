...
class MoodList extends React.Component{
    ...
    handleCloseIntensityModal (){
      this.setState({ showIntensityModal: false });
    }

    handleSubmitIntensity(){
      axios.post('http://localhost:5000/add-logentry', 
        {id: this.state.userSession['id'], mood: this.state.intensity}, 
        { withCredentials: true }
      ).then(res => {
        this.setState({
          showAlert : true
    ...
    onIntensityChange(e) {
      ...
      this.setState(prevState => ({
        intensity: {
          ...prevState.intensity,
          [`${this.state.moodSelected}`]: e.target.textContent
        }
      }))
    ...
    render(){
      ...
      if(this.state.showAlert){
        alert = <Alert severity="success" onClose={()=>{ this.setState({showAlert: false}) }}>Your moods have been successfully logged!</Alert>
      }
