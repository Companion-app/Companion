...
app.post('/register', [
  ...
  check('name').isLength({min: 3, max:30}).trim().escape(),
  check('dob').isISO8601().toDate(),
  check('diagnosis').trim().escape()
], (req, res) => {
...
app.post('/add-logentry', (req, res) => {
  var date = new Date()
  let promises = []
  for(let mood in req.body.intensity) {
    promises.push(db.collection('LogEntries').insertOne({
      ...
      'mood': mood,
      'intensity': req.body.intensity[mood] 
    }))
    promises.push(db.collection('UserDetails').updateOne(
        {'userId' : ObjectId(req.body.id)},
        {
          $inc: {
            [`moods.${mood}.counter`]: 1
          }
        },
        (err, result) => {
          if (err) {
